...
typedef struct _CPifySplash CPifySplash;

/**
 * Callback invoked when the user clicks "Add Folder"
 * or when the splash animation completes and the user proceeds.
 */
typedef void (*CPifySplashCallback)(gpointer user_data);

/**
 * Create a new splash screen widget.
 * The splash shows the CPify logo animating from right to center,
 * with "CPify" and "Offline Vibes" text fading in.
 * 
