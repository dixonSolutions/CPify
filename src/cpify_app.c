...
// Helper to find asset file with fallback paths
char *cpify_find_asset_path(const char *filename) {
  ...
  for (int i = 0; search_dirs[i] != NULL; i++) {
    char *path = g_build_filename(search_dirs[i], filename, NULL);
    if (g_file_test(path, G_FILE_TEST_EXISTS)) {
      g_print("[ASSETS] Found %s at %s\n", filename, path);
      return path;
    }
    g_free(path);
  }
  g_print("[ASSETS] Warning: Could not find asset %s\n", filename);
  return NULL;
}
...
static void update_sidebar_toggle_icon(CPifyApp *p) {
  if (!p || !p->sidebar || !p->sidebar_toggle) return;
  gboolean visible = gtk_widget_get_visible(p->sidebar);
  // Maintain existing logic: visible -> show-symbolic (maybe means "sidebar is shown")
  const char *icon_name = visible ? "sidebar-show-symbolic" : "sidebar-hide-symbolic";
  
  // Try to find SVG asset first
  char *filename = g_strdup_printf("%s.svg", icon_name);
  char *path = cpify_find_asset_path(filename);
  g_free(filename);
  ...
  if (path) {
    ...
    if (texture) {
      ...
      gtk_button_set_child(GTK_BUTTON(p->sidebar_toggle), img);
      g_object_unref(texture);
      g_free(path);
      return;
    }
  ...
  gtk_button_set_icon_name(GTK_BUTTON(p->sidebar_toggle), icon_name);
}
